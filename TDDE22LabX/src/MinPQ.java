...
public class MinPQ<Key_Class> {
    ...
    public void insert(Key_Class x) {
    	...
    	if (count >= pq.length-1) { //Gör längre ifall det behövs
    	    resize(2 * pq.length);
    	}	
    	
    	int curr = ++count; // Ser till att vi börjar på sista platsen
    	pq[curr] = x; //den börjar jiddra vid postincrement, blir nullpointer i och med att vi pekar på vår heap" 
